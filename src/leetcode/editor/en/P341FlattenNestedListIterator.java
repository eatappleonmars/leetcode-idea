...
// 
// Related Topics Stack Tree Depth-First Search Design Queue Iterator 👍 2881 👎
// 987
...
public class P341FlattenNestedListIterator {
    ...
    public class NestedIterator implements Iterator<Integer> {
        ...
        private Deque<Iterator<NestedInteger>> iteratorStack;

        public NestedIterator(List<NestedInteger> nestedList) {
            this.peeked = null;
            this.iteratorStack = new ArrayDeque<>();
            this.iteratorStack.offerLast(nestedList.iterator());
        }
        ...
        private void peek() {
            while (!iteratorStack.isEmpty()) {
                if (!iteratorStack.peekLast().hasNext()) {
                    iteratorStack.pollLast();
                    continue;
                }
                NestedInteger next = iteratorStack.peekLast().next();
                if (next.isInteger()) {
                    this.peeked = next.getInteger();
                    return;
                }
                iteratorStack.offerLast(next.getList().iterator());
            }
        }
    }
    ...
    public static void main(String[] args) {
        List<List<Integer>> test = List.of(List.of(), List.of());
