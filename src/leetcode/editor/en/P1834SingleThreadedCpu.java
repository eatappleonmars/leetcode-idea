...
//processing time, it will choose the task with the smallest resIndex. 
...
// 
// Related Topics Array Sorting Heap (Priority Queue) 👍 682 👎 91
...
public class P1834SingleThreadedCpu {
    ...
    class Solution {
        public int[] getOrder(int[][] tasks) {
            // Waiting tasks have not been enqueued for processing. Sorted by enqueue time.
            Task[] waitingTasks = new Task[tasks.length];
            for (int i = 0; i < tasks.length; i++) {
                waitingTasks[i] = new Task(i, tasks[i][0], tasks[i][1]);
            }
            Arrays.sort(waitingTasks, Comparator.comparingInt(Task::getEnqueueTime));

            // Process queue contains tasks that are enqueued to be processed. Sorted by process time, then id.
            Queue<Task> processQueue = new PriorityQueue<>(Comparator.comparingInt(Task::getProcessTime)
            ...
            while (resIndex < res.length) {
                // currTime changed. Enqueue from waiting tasks
                while (waitIndex < waitingTasks.length && waitingTasks[waitIndex].enqueueTime <= currTime) {
                    processQueue.offer(waitingTasks[waitIndex++]);
                }

                // Fast-forward currTime when CPU becomes idle but tasks are waiting but cannot be enqueued
                if (processQueue.isEmpty()) {
                    currTime = waitingTasks[waitIndex].enqueueTime;
                    continue;
                }

                final Task task = processQueue.poll();
                res[resIndex++] = task.taskId;
                currTime += task.processTime;
            }
        ...
        class Task {
            private final int taskId, enqueueTime, processTime;
            Task(int id, int e, int p) {
                taskId = id;
                enqueueTime = e;
                processTime = p;
            }
            int getTaskId() { return taskId; };
            int getEnqueueTime() { return enqueueTime; }
    ...
    public static void main(String[] args) {
        Solution sol = new P1834SingleThreadedCpu().new Solution();
