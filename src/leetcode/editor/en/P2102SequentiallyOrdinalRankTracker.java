...
// Querying the iᵗʰ best location of all locations already added, where i is 
//the number of times the system has been queried (including the current query).
// 
// For example, when the system is queried for the 4ᵗʰ time, it returns the 4ᵗʰ 
...
//to the system. 
// string get() Queries and returns the iᵗʰ best location, where i is the 
...
//                            // This is the 1ˢᵗ time get() is called, so 
...
//                            // Return the 2ⁿᵈ best location "alps", as it is 
//the 2ⁿᵈ time get() is called.
//tracker.add("orland", 2);   // Add location with name="orland" and score=2 to 
//the system.
//tracker.get();              // Sorted locations: branford, alps, bradford, 
//orland.
//                            // Return "bradford", as it is the 3ʳᵈ time get() 
//is called.
...
// 
// Related Topics Design Heap (Priority Queue) Data Stream Ordered Set 👍 147 👎
// 27
...
public class P2102SequentiallyOrdinalRankTracker {
    ...
    class SORTracker {
        ...
        int topN = 1;
        private Queue<Scenery> topQueue;
        private Queue<Scenery> candidateQueue;

        public SORTracker() {
            // topQueue is a min heap: first element has the lowest score and max lexi order
            this.topQueue = new PriorityQueue<>((s1, s2) -> {
                if (s1.score != s2.score) {
                    return s1.score - s2.score;
                }
                return s2.name.compareTo(s1.name);
            });
            // candidateQueue is a max heap: first element has the highest score and min lexi order
            this.candidateQueue = new PriorityQueue<>((s1, s2) -> {
                if (s1.score != s2.score) {
        ...
        public void add(String name, int score) {
            topQueue.offer(new Scenery(name, score));
            if (topQueue.size() > topN) {
                candidateQueue.offer(topQueue.poll());
            }
        }

        public String get() {
            String name = topQueue.peek().name;
            topN++;
            if (!candidateQueue.isEmpty()) {
                topQueue.offer(candidateQueue.poll());
            }
            return name;
        }
    }
