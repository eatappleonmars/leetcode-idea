...
// 
// Related Topics Depth-First Search Breadth-First Search Union Find Graph 👍 39
//64 👎 258
...
public class P785IsGraphBipartite {
    ...
    class Solution {
        ...
        private boolean dfs(int[][] graph) {
            int[] assignedColors = new int[n]; // 0 - not assigned
            for (int i = 0; i < n; i++) {
                if (assignedColors[i] != 0) {
                    continue; // visited
                }
                assignedColors[i] = -1;
                if (!dfs(i, graph, assignedColors)) {
                    return false;
                }
            }
            return true;
        ...
        private boolean dfs(int index, int[][] graph, int[] assignedColors) {

            int thisColor = assignedColors[index];

            for (int neighborIndex : graph[index]) {
                int neighborColor = assignedColors[neighborIndex];
                if (neighborColor == -thisColor) {
                    continue;
                }
                if (neighborColor == thisColor) {
                    return false;
                }
                assignedColors[neighborIndex] = -thisColor;
                if (!dfs(neighborIndex, graph, assignedColors)) {
                    return false;
                }
            }
            return true;
    ...
    public static void main(String[] args) {
        Solution sol = new P785IsGraphBipartite().new Solution();
