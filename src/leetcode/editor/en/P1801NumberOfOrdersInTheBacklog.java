...
// Related Topics Array Heap (Priority Queue) Simulation 👍 178 👎 186
...
public class P1801NumberOfOrdersInTheBacklog {
    
    //leetcode submit region begin(Prohibit modification and deletion)
    class Solution {
        public int getNumberOfBacklogOrders(int[][] orders) {
            // higher buy price goes first
            Queue<int[]> buyBacklog = new PriorityQueue<>(Comparator.comparingInt(order -> -order[0]));
            // lower sell price goes first
            Queue<int[]> sellBacklog = new PriorityQueue<>(Comparator.comparingInt(order -> order[0]));
            ...
            for (int[] order : orders) {
                ...
                while (!buyBacklog.isEmpty() && !sellBacklog.isEmpty() && buyBacklog.peek()[0] >= sellBacklog.peek()[0]) {
                    int[] buyOrder = buyBacklog.peek();
                    int[] sellOrder = sellBacklog.peek();
                    int amount = Math.min(buyOrder[1], sellOrder[1]);
                    buyOrder[1] -= amount;
                    sellOrder[1] -= amount;
                    if (buyOrder[1] == 0) {
                        buyBacklog.poll();
            ...
            for (int[] buyOrder : buyBacklog) {
                backlogCount = (backlogCount + buyOrder[1]) % 1000000007;
            }

            for (int[] sellOrder : sellBacklog) {
                backlogCount = (backlogCount + sellOrder[1]) % 1000000007;
            }
    ...
    public static void main(String[] args) {
        Solution sol = new P1801NumberOfOrdersInTheBacklog().new Solution();
