...
// 1 <= extraStudents <= 10⁵ 
// 
// Related Topics Array Greedy Heap (Priority Queue) 👍 497 👎 60
...
public class P1792MaximumAveragePassRatio {
    ...
    class Solution {
        public double maxAverageRatio(int[][] classes, int extraStudents) {
            ...
            Queue<int[]> maxGradientQueue = new PriorityQueue<>(Comparator.comparingDouble(c -> -gradient(c)));

            for (int[] c : classes) {
                maxGradientQueue.offer(c);
            }

            for (int i = 0; i < extraStudents; i++) {
                int[] c = maxGradientQueue.poll();
                c[0]++;
                c[1]++;
                maxGradientQueue.offer(c);
            }

            double classPassRatio = 0;
            for (int[] c : classes) {
                classPassRatio += (double) c[0] / c[1];
            }
            return classPassRatio / classes.length;
        }

        private double gradient(int[] c) {
            int pass = c[0];
            double n = c[1];
            return (pass + 1) / (n + 1) - pass / n;
        }
    }
    ...
    public static void main(String[] args) {
        Solution sol = new P1792MaximumAveragePassRatio().new Solution();
