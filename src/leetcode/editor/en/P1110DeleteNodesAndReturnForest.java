...
//            sol2(root, null, toDelete, res);
        ...
        private TreeNode sol2(TreeNode root, TreeNode parent, Set<Integer> toDelete, List<TreeNode> res) {
            ...
            // When root becomes (next) parent when processing L and R
            TreeNode nextParent = toDelete.contains(root.val) ? null : root;

            root.left = sol2(root.left, nextParent, toDelete, res);
            root.right = sol2(root.right, nextParent, toDelete, res);

            if (nextParent != null && parent == null) {
                res.add(root);
            }
            return nextParent;
        }
